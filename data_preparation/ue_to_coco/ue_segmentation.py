...
def get_colours():
    ...
    with open(metadata_filelocation + metadata_filename) as csv_file:
    ...
    for colour in dict_colours.keys():
        ...
        if bbox_list:
            for bbox in bbox_list:
                if bbox[0] < x_min : x_min = bbox[0]
                if bbox[1] < y_min : y_min = bbox[1]
                if bbox[0] + bbox[2] > x_max : x_max = bbox[0] + bbox[2]
                if bbox[1] + bbox[3] > y_max : y_max = bbox[1] + bbox[3]
